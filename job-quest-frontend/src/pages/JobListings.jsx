...
const JobListings = () => {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      setIsLoading(true);
      const jobsResponse = await axios.get("http://localhost:8084/api/v1/jobs");
  ...
  const applyForJob = async (formData) => {
    try {
      const applyResponse = await axios.post("http://localhost:8083/api/v1/applications", formData);
  ...
  const deleteJob = async (job) => {
    ...
    try {
      const deleteResponse = await axios.delete(`http://localhost:8084/api/v1/jobs/${job.id}`);

      if (deleteResponse.status === 204) {
        const removeResponse = await axios.post(
          `http://localhost:8082/api/v1/recruiters/${userData.email}/removejob`,
          job.id
        );
